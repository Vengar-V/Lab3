...
int main() {
	...
	do {
		...
		cout << " оцінкою з фізики/спецільністю/прізвищем" << endl;
		...
		case 5:
			...
			if ((found = BinarP(s, N, prizv, specialnist, physics)) != -1)
				cout << "Знайдено студента з індексом " << found + 1 << endl;
			else
...
void Create(Student* s, const int N)
{
	...
	for (int i = 0; i < N; i++)
	{
		...
		cout << "Інформатика: "; cin >> s[i].informatik;
		cout << endl;
	}
}
...
void Print(Student* s, const int N)
{
	...
	for (int i = 0; i < N; i++)
	{
		...
		cout << "| " << setw(9) << left << s[i].prizv
			...
			<< "| " << setw(12) << left << s[i].informatik << "|" << endl;
	}
	cout << "==================================================================================="
		<< endl;
	cout << endl;
		...
		for (j = i - 1;
			j >= 0 && ((s[I[j]].physics < s[value].physics) ||
				(s[I[j]].physics == s[value].physics &&
					s[I[j]].specialnist > s[value].specialnist) ||
				(s[I[j]].physics == s[value].physics &&
					s[I[j]].specialnist == s[value].specialnist &&
					s[I[j]].prizv > s[value].prizv)); j--)
		{
			I[j + 1] = I[j];
		}
		I[j + 1] = value;
...
void PrintIndex(Student* s, int* I, const int N) {
	...
	for (int i = 0; i < N; i++)
	{
		...
		cout << "| " << setw(9) << left << s[I[i]].prizv
			...
			<< "| " << setw(12) << left << s[I[i]].informatik << "|" << endl;
	}
	cout << "==================================================================================="
		<< endl;
	cout << endl;
